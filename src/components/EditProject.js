...
import { getProjectById, updateProjectById } from "../utils/apiCallActions";
...
const EditProject = (props) => {
  ...
  useEffect(() => {
    ...
    if (loggedUser) {
      ...
      props?.getProjectById(id, navigate);
    }
  }, []);

  useEffect(() => {
    const getData = props.editProject;
    setData({
      projectName: getData?.projectName || "",
      description: getData?.description || "",
      teamMember: getData?.teamMember || [],
    });
  }, [props.editProject]);
  ...
  const handleOnchange = (e) => {
    if (e.target.name === "projectName") {
      ...
      setValidation({ ...validation, projectName: false, invalidEmail: false });
  ...
  const handleCreateClick = () => {
    ...
    if (isValid) {
      props.updateProjectById(id, data, navigate);
    }
  };
        ...
        <div>
          ...
          >
            <Grid2 size={{ md: 8, sm: 8, xs: 12 }}>
              <h4 className="login-text-header">EDIT PROJECT</h4>
...
const mapStateToProps = (state) => {
  return {
    ...
    editProject: state?.projectData?.editProject,
  };
};
